defmodule Fade.Diagnostic.Probes.QueueHighFlowProbe do
  ...
  def execute(config, snapshot) do
    ...
    probe_data = [
      ...
      ProbeData.new(
        property_name: "queue_high_flow_threshold",
        property_value: config.probes.queue_high_flow_threshold
      )
    ]

    if snapshot.messages.incoming.total >= config.probes.queue_high_flow_threshold do
      article =
        KnowledgeBaseArticle.new(
          reason: "Messages being published to broker exceeds the specified threshold."
        )
      ...
      article =
        KnowledgeBaseArticle.new(
          reason: "Messages being published does not exceed the specified threshold."
        )
  ...
  def get_metadata do
    id =
      "Fade.Diagnostic.Probes.QueueHighFlowProbe"
