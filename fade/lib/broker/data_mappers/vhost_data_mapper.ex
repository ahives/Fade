defmodule Fade.Broker.VirtualHostDataMapper do
    ...
    |> Enum.map(fn vhost ->
      VirtualHostInfo.new(
        ...
        packet_bytes_received_details:
          RateDataMapper.map_rate(vhost["packet_bytes_received_details"]),
        packet_bytes_sent: vhost["packet_bytes_sent"],
        packet_bytes_sent_details: RateDataMapper.map_rate(vhost["packet_bytes_sent_details"]),
        total_messages: vhost["total_messages"],
        message_details: RateDataMapper.map_rate(vhost["message_details"]),
        unacknowledged_messages: vhost["unacknowledged_messages"],
        unacknowledged_message_details:
          RateDataMapper.map_rate(vhost["unacknowledged_message_details"]),
        ready_messages: vhost["ready_messages"],
        ready_message_details: RateDataMapper.map_rate(vhost["ready_message_details"])
      )
    end)
  end
  ...
  defp map_message_stats(data) do
    MessageStats.new(
      ...
      unroutable_message_details: RateDataMapper.map_rate(data["unroutable_message_details"]),
      total_message_gets: data["total_message_gets"],
      message_get_details: RateDataMapper.map_rate(data["message_get_details"]),
